...
async def fetch_temperature(city_name: str) -> float:
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(
                status_code=e.response.status_code, detail=f"Error fetching temperature: {str(e)}"
            )
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to fetch temperature: {str(e)}"
            )


@router.get("/", response_model=List[schemas.Temperature])
async def fetch_all_temperatures(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[schemas.Temperature]:
...
async def fetch_temperatures_by_city(city_id: int, skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[schemas.Temperature]:
    temperatures = await crud.get_temperature_by_city(db, city_id=city_id, skip=skip, limit=limit)
    return temperatures


@router.post("/update/", response_model=List[schemas.Temperature])
async def update_temperatures(db: AsyncSession = Depends(get_db)) -> List[schemas.Temperature]:
